...
def eyedisease(request):
    if request.method == 'POST':
        try:
            ...
            if disease_type == 'Cataract':
                model = load_model("prognosis/models/cataract_model.h5")
                ...
                disease_descriptions = {
                    ...
                    1: 'Your eyes are in healthy condition'
                }
            elif disease_type == 'Diabetic Retinopathy':
                model = load_model("prognosis/models/dr_model.h5")
                CLASSES = {0: 'Diabetic Retinopathy', 1: 'Normal'}
                disease_descriptions = {
                    0: 'Your eyes are affected by Diabetic Retinopathy, it causes by diabetes characterized by damage to the blood vessels in the retina, leading to vision impairment or blindness.',
                    1: 'Your eyes are in healthy condition'
                }
            elif disease_type == 'Glaucoma':
                model = load_model("prognosis/models/glaucoma_model.h5")
                CLASSES = {0: 'Glaucoma', 1: 'Normal'}
                disease_descriptions = {
                    0: 'Your eyes are affected by Glaucoma, it causes by increased pressure within the eye, which can damage the optic nerve and result in vision loss or blindness.',
                    1: 'Your eyes are in healthy condition'
                }
            # Make prediction
